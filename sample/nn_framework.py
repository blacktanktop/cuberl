...
class Trainer():
    ...
    def __init__(self, buffer_size=1024, batch_size=32,
                 gamma=0.9, report_interval=10, log_dir="", max_steps=5):
    ...
    def train_loop(self, env, agent, episode=200, initial_count=-1,
        ...
        for i in range(episode):
            ...
            step_count = 0
            scramble_action = env.get_scramble_action()
            self.episode_begin(i, agent, scramble_action)
            for step in range(self.max_steps):
                ...
                self.step(i, action_name, step_count, agent, e)
                reward_all += reward
                s = n_state
                step_count += 1
...
class Observer():
    ...
    def action2name(self, action):
        action_name = self._env.action2name(action)
        return action_name

    def get_scramble_action(self):
        scramble_action = self._env.get_scramble_action()
        return scramble_action

    def get_scramble_size(self):
        return self._env.scrambleSize

    def config(self, views, flat, render, scramble_size):
        self._env.config(views, flat, render, scramble_size)
...
class Logger():
    ...
    def plot(self, name, values, interval=10, output='./output.png'):
